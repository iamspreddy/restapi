...
Select Dates for Leave
    Click On Duration DatePicker
    Click Element    (//table[@class="ant-picker-content"])[2]//tbody/tr[2]/td[1]
    Click Element    (//table[@class="ant-picker-content"])[2]//tbody/tr[2]/td[2]
    Click Element    (//span[text()="Leave time"])
...
#    @{leave_list_values}=   Get List Items    xpath://div[contains(@class, 'MyLeaveTable')]//tbody[@class="ant-table-tbody"]//tr[2]//td//span[text()]
#    Log    @{leave_list_values}
...
Select Date
    [Arguments]    ${month}  ${year}   @{user_Days}
    #Month selection
    Click Element   xpath:(//div[@class="ant-picker-header-view"]//button)[1]
#More Genneric
    Select Month    ${month}
#Task Related
#    Click Element    xpath:(//table[@class="ant-picker-content"]//tbody)[2]//tr[2]/td[2]
    #Year selection
    Click Element   xpath:(//div[@class="ant-picker-header-view"]//button)[2]
#More Genneric
    Select Year    ${year}
#    #Task Related
#    Click Element    xpath:(//table[@class="ant-picker-content"])[2]//tbody//tr[2]//td[2]
#More Generic
    Select Days     @{user_Days}
#Task Related
#    ##Day Selection for task purpose
#    Click Element    xpath:(//table[@class="ant-picker-content"])[2]//tbody//tr[2]//td[2]
#    Click Element    xpath:(//table[@class="ant-picker-content"])[2]//tbody//tr[2]//td[3]
Select Month
    [Arguments]    ${user_month}
    FOR    ${tr_index}    IN RANGE    1    5
        FOR    ${td_index}    IN RANGE    1    4
            ${xpath_month}=    Get Text    xpath=(//table[@class="ant-picker-content"]//tbody)[2]//tr[${tr_index}]/td[${td_index}]
            Run Keyword If    '${user_month}' == '${xpath_month}'
                Click Element    xpath=(//table[@class="ant-picker-content"]//tbody)[2]//tr[${tr_index}]/td[${td_index}]
                Exit For Loop
            END
        END
        Run Keyword And Return    Should Continue For Loop    '${user_month}' != '${xpath_month}'
    END
Select Year
    [Arguments]    ${user_year}
    FOR    ${tr_index}    IN RANGE    1    5
        FOR    ${td_index}    IN RANGE    1    4
            ${xpath_year}=    Get Text    xpath=(//table[@class="ant-picker-content"]//tbody)[2]//tr[${tr_index}]/td[${td_index}]
            Run Keyword If    '${user_year}' == '${xpath_year}'
                Click Element    xpath=(//table[@class="ant-picker-content"]//tbody)[2]//tr[${tr_index}]/td[${td_index}]
                Exit For Loop
            END
        END
        Run Keyword And Return    Should Continue For Loop    '${user_year}' != '${xpath_year}'
    END
Select Days
   [Arguments]    @{user_Days}
    FOR    ${day}    IN    @{user_Days}
        FOR    ${tr_index}    IN RANGE    1    5
            FOR    ${td_index}    IN RANGE    1    4
                ${xpath_day}=    Get Text    xpath=(//table[@class="ant-picker-content"]//tbody)[2]//tr[${tr_index}]/td[${td_index}]
                IF    '${day}' == '${xpath_day}'
                    Click Element    xpath=(//table[@class="ant-picker-content"]//tbody)[2]//tr[${tr_index}]/td[${td_index}]
                    Exit For Loop
                END
            END
            Run Keyword And Return    Should Continue For Loop    '${day}' != '${xpath_day}'
        END
    END
...
Login to HR-OS Portal
    ...
    Click Time Off Option
    General Sleep slowness of site    10

    Click Request Time OFF
    General Sleep slowness of site    6
    Leave Type Selection    PTO
    General Sleep slowness of site    6
    Page Should Contain    Note
    Click Element    //h5[text()="Note"]
    Enter Subject Text    This is for Automation Testing
    Click On Duration DatePicker
    Sleep    8
##    Input Text  xpath://span[text()='Select the days you want to take off']     05/25/2023
##    Input Text  xpath://span[text()='Select the days you want to take off']     05/26/2023
    Click Element    xpath:(//table[@class="ant-picker-content"])[2]//tbody//tr[4]//td[5]
    Click Element    xpath:(//table[@class="ant-picker-content"])[2]//tbody//tr[4]//td[6]
##    Select Date    May  2023    27  28
##    Select Dates For Leave
    General Sleep Slowness Of Site    4
    Page Should Contain    Note
    Click Element    xpath://h5[text()="Note"]
    Enter Description Text    Hello World
    Sleep    10
    Click Element    xpath://span[text()="Description"]
    ...
    Verify Leave Submitted Successfully
    Sleep    10
    Execute JavaScript    window.open('${url_gmail}', '_blank')
    Sleep    10
    @{handles}    Get Window Handles
    @{handles}    Create List    @{handles}
    ${first_handle}    Set Variable    ${handles}[0]
    Log    First Handle: ${first_handle}
##    Terralogic Gmail Login    ${username}    ${password}
    ...
    Withdraw Leave Request Validation
    Capture Page Screenshot    spimages=totawqlpage.png
    Logout HR_OS
    Switch Window   ${handles}[1]
    Logout Terralogic
